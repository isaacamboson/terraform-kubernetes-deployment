...
resource "aws_autoscaling_group" "k8s_master_asg" {
  name                      = "${local.ApplicationPrefix}_master_${var.environment}"
  desired_capacity          = 1
  max_size                  = 1
  min_size                  = 1
  ...
  launch_template {
    id      = aws_launch_template.clixx-k8s-master-LT.id
    version = "$Latest"
  }
  ...
  tag {
    key                 = "Name"
    value               = "${local.ApplicationPrefix}_master_${var.environment}"
...
resource "aws_launch_template" "clixx-k8s-master-LT" {
  name                   = "${local.ApplicationPrefix}-master-LT"
  image_id               = data.aws_ami.k8s_ami.image_id
  instance_type          = var.EC2_Components["instance_type"]
  key_name               = "private-key-kp"
  user_data              = base64encode(data.template_file.k8s-master-node-bootstrap.rendered)
  ...
  tags = {
    Name = "${local.ApplicationPrefix}_master"
  }
